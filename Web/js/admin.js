...
/* --- HÀM MỚI: ĐIỀU KHIỂN MENU MOBILE (BUNG RA BUNG VÔ) --- */
...
function attachProductActionButtons() {
    document.querySelectorAll("#product-table-body .btn-edit").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            // ===== BẮT ĐẦU SỬA (HÀM NÀY) =====
            // document.getElementById("product-image").value = product.image; // DÒNG NÀY GÂY LỖI, XÓA BỎ
            // Bạn không thể gán giá trị cho input type="file" một cách tự động.
            // Logic submit (ở hàm dưới) sẽ tự hiểu là giữ ảnh cũ nếu không có file mới được chọn.
...
// ===== BẮT ĐẦU SỬA (HÀM NÀY) =====
// HÀM NÀY ĐƯỢC VIẾT LẠI HOÀN TOÀN ĐỂ HỖ TRỢ FILE UPLOAD (BASE64)
function attachProductFormEvents() {
    ...
    // Thêm 'async' vì FileReader hoạt động bất đồng bộ
    form.addEventListener("submit", async (e) => { 
        ...
        const descShort = document.getElementById("product-desc-short").value.trim();
        const descLong = document.getElementById("product-desc-long").value.trim();

        const imageInput = document.getElementById("product-image");
        const file = imageInput.files[0]; // Lấy tệp tin người dùng chọn
        let imageUrl = ""; // Biến để lưu trữ đường dẫn ảnh (hoặc Base64)

        if (file) {
            // Trường hợp 1: Người dùng CÓ chọn tệp ảnh mới
            // Chúng ta cần chuyển nó sang Base64
            try {
                imageUrl = await new Promise((resolve, reject) => {
                    const reader = new FileReader();
                    reader.readAsDataURL(file); // Đọc tệp tin thành chuỗi Base64
                    reader.onload = () => resolve(reader.result); // Trả về chuỗi khi hoàn thành
            ...
            // Trường hợp 2: Người dùng KHÔNG chọn tệp mới, và đang SỬA
            // Chúng ta phải giữ lại ảnh cũ
            const products = getProducts();
            const oldProduct = products.find(p => p.id === id);
            if (oldProduct) {
                imageUrl = oldProduct.image; // Giữ nguyên ảnh cũ
            } else {
                alert("Lỗi: Không tìm thấy sản phẩm gốc để giữ ảnh.");
                return;
            }
        } else {
            // Trường hợp 3: Người dùng KHÔNG chọn tệp, và đang THÊM MỚI
            alert("Vui lòng chọn một hình ảnh cho sản phẩm mới!");
            return;
        }

        // Kiểm tra validation
        if (!id || !name || !category || !price) { 
            alert("Vui lòng nhập đầy đủ các trường bắt buộc (Mã, Tên, Loại, Giá)!"); 
            return; 
        }
        // imageUrl đã được xử lý ở trên, nên nó sẽ luôn có giá trị
        // nếu logic đi đến được đây.
        ...
        const product = { 
            ...
            category: category, 
            price: price, 
            image: imageUrl, // <-- SỬ DỤNG BIẾN IMAGEURL MỚI
...
function resetProductForm() {
    ...
    // ===== BẮT ĐẦU SỬA (HÀM NÀY) =====
    document.getElementById("product-image").value = null; // Dùng 'null' để reset input type="file"
    // ===== KẾT THÚC SỬA (HÀM NÀY) =====
...
function loadOrderTable(filterDateStart = '', filterDateEnd = '', filterStatus = '') {
    ...
    orders.sort((a, b) => new Date(b.orderDate) - new Date(a.date));
